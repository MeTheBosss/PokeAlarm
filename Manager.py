...
# begin DMS eliminate raids in Great America  *************************************************************************************
        ...
        log.info("DMS egg gym name {}".format(gname))

        if gname=='Ornate Elevated Vase':
            if self.__quiet is False:
                log.info("DMS Egg at {} ignored: GA Ornate Elevated Vase".format(gname))
            return

        if gname=='The Demon':
            if self.__quiet is False:
                log.info("DMS Egg at {} ignored: GA The Demon".format(gname))
            return

        if gname=='Frog Leg Pinwheel':
            if self.__quiet is False:
                log.info("DMS Egg at {} ignored: GA Frog Leg Pinwheel".format(gname))
            return

        if gname=='On the Mic Karaoke':
            if self.__quiet is False:
                log.info("DMS Egg at {} ignored: GA On the Mic Karaoke".format(gname))
            return

        if gname=='Boomerang Bay Torches':
            if self.__quiet is False:
                log.info("DMS Egg at {} ignored: GA Boomerang Bay Torches".format(gname))
            return

        if gname=='Water Mushrooms':
            if self.__quiet is False:
                log.info("DMS Egg at{} ignored: GA Water Mushrooms".format(gname))
            return

        if gname=='Castaway Creek':
            if self.__quiet is False:
                log.info("DMS Egg at {} ignored: GA Castaway Creek".format(gname))
            return
